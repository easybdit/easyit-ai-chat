...
class EAIC_Product_AI {
	 ...
	 * @return string
	 */
	public static function system_prompt( $product_data, $user_name = '' ) {
		$base  = 'You are a helpful product assistant for an online store. ';

		if ( '' !== $user_name ) {
			$first = explode( ' ', trim( $user_name ) )[0];
			$base .= "You are talking to {$first}. Address them by their first name occasionally to be friendly. ";
		}

		$base .= 'Answer questions about products using ONLY the product data provided below. ';
		$base .= 'When mentioning a product by name, always format it as a markdown link using its URL field, like: [Product Name](URL). ';
